...
def get_card_links(page):
    """Collect all place URLs from search results. More reliable than clicking cards."""
    links = []
    seen_hrefs = set()

    # Method 1: href from a/button cards
    ...
    # Method 2: Find all links in the feed that point to /place/
    if len(links) < 3:
        feed = page.query_selector('div[role="feed"]')
        if feed:
            all_links = feed.query_selector_all('a[href*="/place/"]')
            for a in all_links:
                href = a.get_attribute("href")
                if href and href not in seen_hrefs:
                    seen_hrefs.add(href)
                    name = a.get_attribute("aria-label") or a.inner_text().strip()[:60]
                    links.append({"href": href, "name": name})

    # Method 3: Extract from page source via JS
    if len(links) < 3:
        hrefs = page.evaluate("""() => {
            const links = document.querySelectorAll('a[href*="/place/"]');
            return Array.from(links).map(a => ({
                href: a.href,
                name: a.getAttribute('aria-label') || a.innerText.trim().substring(0, 60)
            }));
        }""")
        for item in hrefs:
...
def extract_one_listing(page, card_index):
    ...
    try:
        ...
        print(f"  ⚠ Error at card {card_index}: {type(e).__name__}: {e}", file=sys.stderr)
        return None
    finally:
        # Navigate back to search results after each card click
        # This fixes the DOM state so next card can be clicked
        try:
            # Press Escape to close detail panel and return to list
            page.keyboard.press("Escape")
            time.sleep(1)
            # If still on detail page, use browser back
...
def extract_from_detail(page):
    """Extract data from a detail page (when already on a place URL). Returns dict or None."""
    try:
        ...
        print(f"  ⚠ Error extracting detail: {type(e).__name__}: {e}", file=sys.stderr)
        return None


def extract_listings(page, max_results):
    """Extract business info by visiting URLs one by one (more reliable than clicking cards)."""
    # Step 1: Collect all place URLs from search results
    print("  🔗 Collecting place URLs from search results...")
    all_links = get_card_links(page)
    print(f"  📎 Found {len(all_links)} place URLs")

    if not all_links:
        print("  ⚠ No place URLs found, falling back to click-based extraction...")
        return extract_listings_click(page, max_results)
    ...
    for i, link in enumerate(all_links):
        ...
        href = link["href"]
        name_preview = link.get("name", "")[:40]

        try:
            # Navigate directly to place URL
            page.goto(href, wait_until="domcontentloaded", timeout=30000)
            data = extract_from_detail(page)
            ...
            if data and data["Name"] and data["Name"] not in seen_names:
                ...
                reason = "no data" if data is None else ("duplicate" if data and data["Name"] in seen_names else "empty name")
                print(f"  ⏭ [{i+1}] {name_preview}: skipped ({reason})")
        except Exception as e:
            print(f"  ⚠ [{i+1}] {name_preview}: {type(e).__name__}", file=sys.stderr)

    return results


def extract_listings_click(page, max_results):
    """Fallback: Extract business info by clicking cards one by one."""
    results = []
    seen_names = set()
    i = 0
    fails = 0
    ...
    while len(results) < max_results and fails < max_fails:
        cards = get_listing_cards(page)
        if i >= len(cards):
            print(f"  ⏹ No more cards (total: {len(cards)})")
            break
